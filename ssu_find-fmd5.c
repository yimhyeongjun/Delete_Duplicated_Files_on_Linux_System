...
const char* transtoMD5(char* pathname, char* md_buf); // pathname의 경로를 가진 파일의  MD5 파일을 추출해주는 함수
...
void bfs(char *target_path, int depth){
        ...
        char search_md5[md5HASH]; // md5 추출하여 저장할 배열
	...
	// 탐색할 경로에 접근해도 되는지 확인
		...
		//	printf("search / sub : %s, num : %d, r : %d\n", target_path, k,r);
			...
			continue;
                ...
                // 정규 파일인지 검사 -> 맞다면 file_extension 검사 -> size 검사 -> 같다면 md5 해쉬값 검사 
			...
			r++;
                        if(!strcmp(file_extension,"all")){ // 파일 확장자가 all이면 전부 탐색
                                ...
                                {
                                        // md5 추출하여 search_md5에 저장
                                        strcpy(search_md5,transtoMD5(search_path,md_buf));
					// 리스트에 삽입
		                        setInsert(&list, search_md5, statbuf.st_size, search_path, get_time(statbuf.st_mtime), get_time(statbuf.st_atime), depth+1, dupfSort);
				...
				;
                                ...
                                if(statbuf.st_size >= min_size && statbuf.st_size <= max_size){ // 파일의 크기가 주어진 범위 안에 속하면
                                        //md5 추출하여 md5_search에 저장        
                                        strcpy(search_md5, transtoMD5(search_path, md_buf));
                                       // 리슽에 삽입
				       	setInsert(&list, search_md5, statbuf.st_size,search_path, get_time(statbuf.st_mtime), get_time(statbuf.st_atime), depth+1, dupfSort);     // mtime, atime 문자열 버전
                                }
                        }else ;
...
// md5 추출 함수
const char* transtoMD5(char* pathname, char* md_buf){
        ...
        if(strcpy(md_buf, ret) <= 0){
                fprintf(stderr,"File %s Transfer to md5 error\n", pathname);
                exit(-1);
        }
...
// set 삽입 함수
void setInsert(set_list* list, char* md5, long size, char* search_path, char* mtime, char* atime, int depth, int (*dupfSort)(int l1, int l2, char * p1, char * p2)){
	...
	// 존재하는 set중  정렬 조건에 부합하는 위치에 커서를 이동시킴
        while(cursor != NULL && list->comp(size, cursor->size) < 1){
                ...
                if(!strcmp(cursor->hash, md5)){ // size가 같은데 hash 값까지 같다면 중복 파일 리스트에 저장
